...
public class GenerateQuestions {
    ...
    public synchronized void generateNewExpression() {
        question = "";
            ...
            question += Integer.toString((int) (Math.random() * 10e5 % Math.pow(10, digitSize)))
                    + (i < questionLength - 1
                            ? operatorPrecedenceTable
                                    .keySet()
                                    .toArray()[(int) (Math.random() * 10e5) % operatorPrecedenceTable.size()]
                            : "");
        }
        try {
    ...
    public synchronized void setExpressionLength(int length) {
        questionLength = length;
    }
    public synchronized void setDigitSize(int size) {
        digitSize = size;
    }
    public synchronized void addOperator(String symbol, int precendence) {
