...
public class StudentMS extends JFrame implements ChangeListener,
        ActionListener {
    ...
    public StudentMS(String frameTitle) {
        super(frameTitle);
        setResizable(true);
        setSize(400, 400);
    ...
    public static void main(String args[]) {
        StudentMS outputScreen = new StudentMS("Case Study");
    }
    ...
    public String getStudentGrade() {
        ...
        if (studentDetails.physicsMarks < passMarks
            ...
            if (avgMarks >= passMarks && avgMarks < secondClass) {
                grade = "PassClass";
            } else if (avgMarks >= secondClass && avgMarks < firstClass) {
                grade = "SecondClass";
            } else if (avgMarks >= firstClass && avgMarks < topScore) {
                grade = "FirstClass";
            } else {
                grade = "Distinction";
            }
    ...
    public void stateChanged(ChangeEvent e) {
        switch (tabbedPane.getSelectedIndex()) {
            case 3:
                studentPassed.setText(getNumberPasses());
                break;
            case 4:
                studentTopper.setText(getClassTopper());
                break;
        }
    }
...
class Registration {
    ...
    public int addStudent(String studentName, int physicsMarks, int biologyMarks, int mathsMarks) {
        int id = 0;
        try {
...
//Function to get the next ID available
    public static int getNextID() {
        int id = 0;
        try {
            RandomAccessFile studentIDFile = new RandomAccessFile(Registration.idFile, "rw");
            if (studentIDFile.length() == 0) {
                id = 0;
            } else {
                id = studentIDFile.readInt();
            }
            id++;
            studentIDFile.close();
        } catch (IOException e) {
